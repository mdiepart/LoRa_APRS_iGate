...
bool FTPTask::setup(System &system) {
  for (Configuration::Ftp::User user : system.getUserConfig()->ftp.users) {
    system.log_debug(getName(), "Adding user to FTP Server: %s", user.name.c_str());
...
bool FTPTask::loop(System &system) {
  ...
  if (configWasOpen && _ftpServer.countConnections() == 0) {
    system.log_warn(getName(), "Maybe the config has been changed via FTP, lets restart now to get the new config...");
    ESP.restart();
  }

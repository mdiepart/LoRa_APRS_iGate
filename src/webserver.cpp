...
void webserver::serve(WiFiClient &client, System &system) {
  ...
  system.getLogger().log(logging::LoggerLevel::LOGGER_LEVEL_INFO, String("webServer"), "start line : %s", start_line.c_str());
  ...
  if (known_resources.find(target.getResource()) == known_resources.end()) {
    ...
    system.getLogger().log(logging::LoggerLevel::LOGGER_LEVEL_INFO, String("webServer"), "Found weird target, we should not be here...");

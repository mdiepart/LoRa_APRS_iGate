...
void WiFiEvent(WiFiEvent_t event) {
  ...
  case SYSTEM_EVENT_STA_CONNECTED:
    logger.info(WIFI_EVENT, "WiFi Connected");
    break;
  case SYSTEM_EVENT_STA_GOT_IP:
    logger.info(WIFI_EVENT, "WiFi MAC: %s", WiFi.macAddress().c_str());
    logger.info(WIFI_EVENT, "WiFi IPv4: %s", WiFi.localIP().toString().c_str());
    logger.info(WIFI_EVENT, "WiFi Gateway: %s", WiFi.gatewayIP().toString().c_str());
    logger.info(WIFI_EVENT, "WiFi DNS1: %s", WiFi.dnsIP().toString().c_str());
    logger.info(WIFI_EVENT, "WiFi DNS2: %s", WiFi.dnsIP(1).toString().c_str());
    logger.info(WIFI_EVENT, "WiFi Hostname: %s", WiFi.getHostname());
    break;
  case SYSTEM_EVENT_STA_DISCONNECTED:
    logger.info(WIFI_EVENT, "WiFi Disconnected");
    break;
  case SYSTEM_EVENT_STA_STOP:
    logger.info(WIFI_EVENT, "WiFi Stopped");
    break;
  case SYSTEM_EVENT_ETH_START:
    logger.info(WIFI_EVENT, "ETH Started");
    break;
  case SYSTEM_EVENT_ETH_CONNECTED:
    logger.info(WIFI_EVENT, "ETH Connected");
    break;
  case SYSTEM_EVENT_ETH_GOT_IP:
    logger.info(WIFI_EVENT, "ETH MAC: %s", ETH.macAddress().c_str());
    logger.info(WIFI_EVENT, "ETH IPv4: %s", ETH.localIP().toString().c_str());
    logger.info(WIFI_EVENT, "ETH Gateway: %s", ETH.gatewayIP().toString().c_str());
    logger.info(WIFI_EVENT, "ETH DNS1: %s", ETH.dnsIP().toString().c_str());
    logger.info(WIFI_EVENT, "ETH DNS2: %s", ETH.dnsIP(1).toString().c_str());
    logger.info(WIFI_EVENT, "ETH Hostname: %s", ETH.getHostname());
    if (ETH.fullDuplex()) {
      logger.info(WIFI_EVENT, "ETH FULL_DUPLEX");
    }
    logger.info(WIFI_EVENT, "ETH Speed: %dMbps", ETH.linkSpeed());
    eth_connected = true;
    break;
  case SYSTEM_EVENT_ETH_DISCONNECTED:
    logger.warn(WIFI_EVENT, "ETH Disconnected");
    eth_connected = false;
    break;
  case SYSTEM_EVENT_ETH_STOP:
    logger.warn(WIFI_EVENT, "ETH Stopped");
    eth_connected = false;
    break;
  default:
    break;
